...
class IA_Page {
    ...
    public function __call($name, $arguments)
    {
        if (file_exists(APPPATH . "models/{$this->slug}_model.php"))
        {
            ...
            {
                show_error("Function \"{$name}()\" does not exist in \"\${$this->slug}_model\"s");
            }
        }
        else
        {
            show_error("Tried calling \"\${$this->slug}_model->{$name}()\", but model does not exist.");
        }
    }
